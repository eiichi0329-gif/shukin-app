...
const MANUAL_PENDING_KEY  = 'coll-manual-pending-v1'; // GAS未確認の手動追加ID
...
// 金種データのインメモリキャッシュ（localStorageより優先、起動時に初期化・GAS同期で更新）
// GAS を正として無条件に反映するため、端末間で常に最新データを参照できる
let gasDenomData = {}; // startApp() で loadDenomStorage() から初期化
const DENOMINATIONS = [
...
// 手動追加レコードのIDを追跡（GASが確認するまで同期での削除を防ぐ）
// localStorage に永続化するためページリロード後も保護が継続する
function loadPendingManualIds() {
    try { return new Set(JSON.parse(localStorage.getItem(MANUAL_PENDING_KEY) || '[]')); } catch { return new Set(); }
}
function savePendingManualIds(s) {
    localStorage.setItem(MANUAL_PENDING_KEY, JSON.stringify([...s]));
}
let pendingManualIds = loadPendingManualIds();
...
// 振込入金済み金額を返す（amount 未設定 = 全額とみなす、未入金は 0）
function transferredAmount(key, r) {
    const ts = transferState[key];
    if (!ts) return 0;
    const amt = ts.amount;
    if (amt === undefined || amt === null) return effectiveAmount(key, r); // 旧形式 or 全額
    return Number(amt);
}
...
// 集金済み金額との差分（振込入金一部払い・追加注文も考慮）
function displayAmount(key, r) {
    const base         = effectiveAmount(key, r);
    const collectedAmt = checked[key]?.collectedAmount || 0;
    const transferAmt  = transferredAmount(key, r);
    // リセット運用：集金後にExcelが0になった場合は差分計算せず全額表示
    if (checked[key]?.cycleReset) return base;
    const totalReceived = collectedAmt + transferAmt;
    if (totalReceived > 0 && base > totalReceived) return base - totalReceived;
    return base;
}

// Excel金額 === 集金済み金額（現金＋振込）→ 完全集金済み
...
function isFullyCollected(key, r) {
    ...
    const baseAmt      = effectiveAmount(key, r);
    const transferAmt  = transferredAmount(key, r);
    // 旧データ互換：collectedAmount が記録されていない場合（旧形式チェック）は集金済みとみなす
    if (collectedAmt === undefined || collectedAmt === null) return true;
    // 合計受取額が 0 の場合は金額も 0 のときだけ集金済み
    if (collectedAmt + transferAmt === 0) return baseAmt === 0;
    // リセット運用：集金後にExcelが0にリセットされた状態は集金済みとみなす
    if (checked[key].cycleReset && baseAmt === 0) return true;
    // 現金＋振込の合計が請求額以上なら集金済み
    return baseAmt <= collectedAmt + transferAmt;
}
...
function filteredData() {
    return allData.filter(r => {
        ...
        // 振込入金済み：未集金フィルター ON のとき非表示（一部振込は残額あり → 表示継続）
        if (transferState[key] && effectiveUncollectedOnly) {
            if (transferredAmount(key, r) >= effectiveAmount(key, r)) return false;
        }
...
function renderTable() {
    ...
    data.forEach(r => {
        ...
        const transferAmt     = transferredAmount(key, r);
        const isPartialTransfer = isTransferred && transferAmt < effectiveAmount(key, r);

        // 一部集金状態（isPartiallyCollected）は transferCellHtml の分岐で使うため先に計算する
        // 一部振込がある場合も現金一部集金を許可する
        const canPartial = !isBankCustomer && (!isTransferred || isPartialTransfer);
        ...
        if (isBankCustomer) {
            ...
            if (isPartialTransfer) {
                // 一部振込あり：追加振込ボタンを表示（取消はダイアログ内から）
                transferCellHtml = `<td class="col-transfer"><button class="btn-transfer btn-transfer-partial" data-key="${key}">振込入金</button></td>`;
            } else {
    ...
    // イベント登録（振込入金 取消：全額・一部どちらも）
    tbody.querySelectorAll('td.transfer-done[data-key], td.transfer-partial[data-key]').forEach(cell => {
...
function openPartialCollect(key) {
    ...
    const transferAmt   = transferredAmount(key, record);
    const remaining     = fullAmt - prevCollected - transferAmt;
    ...
    infoEl.innerHTML = `
        ...
        ${prevCollected > 0 ? `<div class="partial-info-row"><span>既集金（現金）</span><strong>¥${fmt(prevCollected)}</strong></div>` : ''}
        ${transferAmt > 0 ? `<div class="partial-info-row"><span>既入金（振込）</span><strong>¥${fmt(transferAmt)}</strong></div>` : ''}
...
function confirmPartialCollect() {
    ...
    // 全額振込済みの場合は現金集金不可（一部振込の場合は残額集金を許可）
    if (transferState[key]) {
        const tAmt = transferredAmount(key, record);
        if (tAmt >= effectiveAmount(key, record)) {
            alert('振込入金（全額）が記録されています。先に振込入金を取り消してください。');
    ...
    const isFullNow    = (newCollected + transferAmt) >= fullAmt;
    const gasKey       = isFullNow && prevCollected === 0 && transferAmt === 0
        ? key                           // 初回で全額（振込なし）→ 通常キー
        : `${key}|${Date.now()}`;       // 一部または再集金、振込あり → 別行
...
function onTransferClick(key) {
    ...
    document.getElementById('transfer-date-input').value =
        new Date().toLocaleDateString('sv-SE', { timeZone: 'Asia/Tokyo' });

    // ラジオボタンを初期化
    document.getElementById('transfer-type-full').checked = true;
    document.getElementById('transfer-amount-row').style.display = 'none';
    document.getElementById('transfer-amount-input').value = '';

    // ラジオ切り替えイベント
    ['transfer-type-full', 'transfer-type-partial'].forEach(id => {
        const el = document.getElementById(id);
        el.onchange = () => {
            const isPartial = document.getElementById('transfer-type-partial').checked;
            document.getElementById('transfer-amount-row').style.display = isPartial ? '' : 'none';
            if (isPartial) {
                const inp = document.getElementById('transfer-amount-input');
                inp.max = fullAmt;
                inp.focus();
            }
        };
    ...
    // 既存の一部振込がある場合の情報表示と取消ボタン制御
    const existingTransfer = transferState[key];
    const existingAmt = existingTransfer
        ? (existingTransfer.amount !== undefined ? Number(existingTransfer.amount) : fullAmt)
        : 0;
    const remainingAfterTransfer = fullAmt - existingAmt;

    let infoHtml = `
        <div class="partial-info-name">${escHtml(effectiveName(key, record))}</div>
        <div class="partial-info-row"><span>請求額</span><strong>¥${fmt(fullAmt)}</strong></div>
    `;
    if (existingAmt > 0 && existingAmt < fullAmt) {
        infoHtml += `
        <div class="partial-info-row"><span>既振込</span><strong style="color:#c2410c">¥${fmt(existingAmt)}</strong></div>
        <div class="partial-info-row partial-info-remaining"><span>残額</span><strong>¥${fmt(remainingAfterTransfer)}</strong></div>
        `;
        // 追加入力の上限を残額に設定
        document.getElementById('transfer-amount-input').max = remainingAfterTransfer;
    } else {
        document.getElementById('transfer-amount-input').max = fullAmt;
    }
    document.getElementById('transfer-info').innerHTML = infoHtml;

    // 既存振込がある場合は取消ボタンを表示
    document.getElementById('transfer-cancel-btn').style.display = existingTransfer ? '' : 'none';
...
function confirmTransferDialog() {
    ...
    const fullAmt   = effectiveAmount(key, record);
    // 既存の一部振込額（ある場合）
    const existingAmt = transferState[key]
        ? (transferState[key].amount !== undefined ? Number(transferState[key].amount) : fullAmt)
        : 0;
    let newTotalAmt; // undefined = 全額

    if (isPartial) {
        const additionalAmt = parseInt(document.getElementById('transfer-amount-input').value) || 0;
        if (additionalAmt <= 0) { alert('振込金額を入力してください'); return; }
        const total = existingAmt + additionalAmt;
        newTotalAmt = total >= fullAmt ? undefined : total; // 合計が全額以上なら全額扱い
    }
    // isPartial = false（全額） → newTotalAmt = undefined のまま

    const recordedAt = new Date().toISOString();
    transferState[key] = newTotalAmt !== undefined
        ? { date: dateVal, recordedAt, amount: newTotalAmt }
        : { date: dateVal, recordedAt };
    saveTransferState();

    const url = getGasUrl();
    if (url) {
        const gasAmount = newTotalAmt !== undefined ? newTotalAmt : fullAmt;
        postToGas(url, {
            action: 'addTransfer',
            record: { ...record, key, amount: gasAmount },
...
function addManualRecord() {
    ...
    // GASが確認するまで同期で削除されないよう、IDを保護リストに追加（localStorage永続化）
    pendingManualIds.add(record.id);
    savePendingManualIds(pendingManualIds);
...
function deleteManualRecord(id) {
    ...
    // 削除されたIDは保護リストからも除去
    if (pendingManualIds.has(id)) { pendingManualIds.delete(id); savePendingManualIds(pendingManualIds); }
...
function switchTab(tab) {
    ...
    if (tab === 'admin') {
        renderAdmin();          // まずキャッシュで即時描画
        fetchDenomFromGas();    // 非同期でGASから最新データを取得し再描画
    }
    if (tab === 'list') {
        renderTable();          // フィルター変更後も常に最新状態を表示
    }
    if (tab === 'delivery') {
        try {
            renderDelivery();
...
function buildDailySection(date, routes, routeMonthData, dateItems) {
    ...
    for (const r of routes) {
        ...
        if ((routeTotals[r] || 0) > 0) {
            const dd = gasDenomData[`${date}|${r}`] || null; // GASから取得した最新データを使用
            let seisaStatus = `<div class="seisa-status seisa-none">未実施</div>`;
            if (dd) {
                const denomTotal  = calcDenomTotal(dd.counts);
                // 保存時の期待値を使用：後から集金データが同期されても合致状態が変わらないよう固定
                // （精査ダイアログで検証した時点の金額で判定するのが正しい）
                const seisaTarget = dd.expected || cash;
...
function openDenomDialog(date, route, base, ca) {
    ...
    const saved    = gasDenomData[cashKey];
...
// 管理タブを開いたとき GAS から最新の金種データを即時取得して表示に反映する
async function fetchDenomFromGas() {
    const url = getGasUrl();
    if (!url) return;
    try {
        ...
        const json = await res.json();
        if (!json.denomData) return;
        let changed = false;
        Object.entries(json.denomData).forEach(([key, remote]) => {
            if (denomRecentlySaved.has(key)) return; // 直近保存キーは保護
            gasDenomData[key] = remote;
            changed = true;
        });
        if (changed) {
            saveDenomStorage({ ...gasDenomData });
            if (currentTab === 'admin') renderAdmin();
        }
    } catch(e) {
        console.warn('現金精査データの取得に失敗:', e);
    }
}
...
function saveDenomDialog() {
    ...
    const entry = { counts, expected, savedAt: new Date().toISOString() };
    storage[key] = entry;
    saveDenomStorage(storage);
    // インメモリキャッシュも即時更新（この端末での保存はすぐに表示に反映）
    gasDenomData[key] = entry;
...
async function syncCheckboxes() {
    ...
    try {
        ...
        if (json.checkedData) {
            ...
            Object.keys(checked).forEach(key => {
                if (!remote[key] && !dirtyKeys.has(key)) {
                    // 一部集金でタイムスタンプ付きキー（key|timestamp）がリモートに存在する場合は基本キーを保護
                    const hasTimestampedVariant = Object.keys(remote).some(rk => rk.startsWith(key + '|'));
                    if (!hasTimestampedVariant) delete checked[key];
                }
            });
        } else {
        ...
        // 現金精査データ: GAS を正として gasDenomData を更新
        // タイムスタンプ比較なしで無条件に反映（端末間の時刻差・反映タイミングに関係なく同一表示を保証）
        // ただし denomRecentlySaved にあるキーはこの端末が直近に保存したもの → POST未反映の可能性があるため保護
        if (json.denomData) {
            let denomChanged = false;
            Object.entries(json.denomData).forEach(([key, remote]) => {
                if (denomRecentlySaved.has(key)) return; // この端末での保存直後は保護
                gasDenomData[key] = remote;
                denomChanged = true;
            });
            if (denomChanged) {
                // localStorageも更新（オフライン時の fallback 用）
                saveDenomStorage({ ...gasDenomData });
                if (currentTab === 'admin') renderAdmin();
            }
        }
        ...
        if (Array.isArray(json.manualData)) {
            ...
            // GASが返してきたIDは「確認済み」→ 保護リストから削除（以後は通常の削除同期が有効になる）
            let pendingChanged = false;
            pendingManualIds.forEach(id => {
                if (remoteIds.has(id)) { pendingManualIds.delete(id); pendingChanged = true; }
            });
            if (pendingChanged) savePendingManualIds(pendingManualIds);

            // ローカルにあってリモートにないレコードを削除（他端末で削除済み）
            // ただし GAS 未確認のレコード（pendingManualIds）は削除しない
            const before = manualRecords.length;
            manualRecords = manualRecords.filter(r => remoteIds.has(r.id) || pendingManualIds.has(r.id));
            allData = allData.filter(r => !r.isManual || remoteIds.has(r.id) || pendingManualIds.has(r.id));
...
async function startApp() {
    ...
    gasDenomData    = loadDenomStorage(); // localStorageで初期化（起動後にGASで上書き）
    ...
    document.getElementById('filter-store').addEventListener('change', e => {
        ...
        if (currentTab === 'msg')      renderMsgTab();
        if (currentTab === 'admin')    renderAdmin();
        if (currentTab === 'delivery') { try { renderDelivery(); } catch(e2) { console.error(e2); } }
    });
